	...
	public void train(DataSet dataSet) {
		...
		while (iterator.hasNext()) {
			...
			if (classAttribute.label) {
                ...
                System.out.println("classAttribute = " + classAttribute.name);
                System.out.println("classAttribute = " + classAttribute.values);
                
                //System.out.println("iterator = " + dataSet.attributes.size());                                
		String get0 = classAttribute.values.get(0);
//                System.out.println("class A = " + get0);
		String get1 = classAttribute.values.get(1);
//                System.out.println("class B = " + get1);
		class1 = get0;
		class2 = get1;
//                dataSet.print();
//                System.out.println(dataSet.instances.clone());
//                dat
                
                List<Instance> instancesChild = new LinkedList<>(dataSet.instances);
                System.out.println("instancesChild = " + instancesChild.size());
                
                treeRoot = decisionTreeLearning(instancesChild, dataSet.attributes, dataSet.instances);
//		printTree();
	}
	...
	private Node decisionTreeLearning(List<Instance> instances, 
			List<Attribute> attributes, List<Instance> parentInstances) {
                ...
                if (instances.isEmpty())
                    {
                        System.out.println("Size empty = " + instances.size());                        
                        return new Node(pluralityValue(parentInstances), true);
                    }
		if (instances.size() < minInstances)
                {
                        System.out.println("minInstances = " + minInstances);
...
//                System.out.println("test numeric = " + (bestAttribute.numeric));
		if (bestAttribute.numeric) {
			...
			for (Instance instance : instances) {
				List<Instance> subset = subsets.getOrDefault(instance.values.get(bestAttribute.name), new LinkedList<>());
				subset.add(instance);
			}
		}
	...
	private double conditionalEntropy(List<Instance> instances, Attribute attribute) {
		...
		LinkedHashMap<String, List<Instance>> subsets = new LinkedHashMap<>();                
//                System.out.println("attribute = " + attribute.values);
                for (String value : attribute.values)
			subsets.put(value, new ArrayList<Instance>());

		for (Instance instance : instances) {                        
			List<Instance> subset = subsets.getOrDefault(
                                instance.values.get(attribute.name), new LinkedList<>());
//                        System.out.println("subset = " + subset);
	...
	private String pluralityValue(List<Instance> instances) {
		...
		for (Instance instance : instances) 
			if (instance.label.equals(class1))
				firstCount++;
//                System.out.println("firstCount = " + firstCount + " =>"+class1);
